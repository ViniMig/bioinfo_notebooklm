...
def retrieve_bmc_paper() -> str:
    ...
    BMC_KEY = constants.BMC_API_KEY
    BMC_BIOINFORMATICS_ISSN = "1471-2105"
    MAX_RESULTS = 1
    START_RESULT_INDEX = 1

    url = f'https://api.springernature.com/meta/v2/json?api_key={BMC_KEY}&callback=&s={START_RESULT_INDEX}&p={MAX_RESULTS}&q=(issn:{BMC_BIOINFORMATICS_ISSN})'
    response = requests.request("GET", url)
    result = response.json()

    paper_url = url in result['records'][0]['url'][0]['value']
